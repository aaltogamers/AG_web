...
| **admin_password**            | Password gating the admin-only analytics API (`/api/analytics/stats`), the Statistics tab in the admin panel, **and** the CMS login (`/api/auth`) | You choose it. Any strong secret. Exposed to the app as the `ADMIN_PASSWORD` environment variable. |
| **agent_api_key**             | Optional key for the AI agent API (`/api/agent/**`, see the main README) | You choose it, e.g. `openssl rand -hex 32`. Exposed to the app as `AGENT_API_KEY`. Leave empty to disable the agent API. |
| **github_app_id**             | GitHub App Application ID                                | See **GitHub App credentials** section below. Exposed to the app as `APP_ID`. |
| **github_app_private_key_base64** | GitHub App private key, base64-encoded PEM          | See **GitHub App credentials** section below. Exposed to the app as `PRIVATE_KEY`. |
| **github_app_installation_id** | GitHub App installation ID on the repo/org             | See **GitHub App credentials** section below. Exposed to the app as `INSTALLATION_ID`. |
| **app_settings**              | Optional extra environment variables for the Next.js app | Key-value map. See **RCON** below. |
...
#### GitHub App credentials (github_app_id, github_app_private_key_base64, github_app_installation_id)

Used by the **CMS** (Decap editor at `/cms`). The CMS password is the same as `admin_password`; once validated, the app uses a GitHub App to issue an installation access token so the CMS can read/write content in this repo via the GitHub API. These three variables are required for the CMS to work.

At runtime they're surfaced to the app as the env vars `APP_ID`, `PRIVATE_KEY`, and `INSTALLATION_ID`.

| Variable | What it is | Where to get it |
|----------|------------|-----------------|
| **github_app_id** | GitHub App’s numeric Application ID | **GitHub** → **Settings** (of the user or org that owns the app) → **Developer settings** → **GitHub Apps** → your app → on the app’s **General** page, the **App ID** is shown near the top. For an org-owned app: **&lt;org&gt; → Settings → Developer settings → GitHub Apps** → your app. |
| **github_app_private_key_base64** | The app’s private key, **base64-encoded** | Same app page → **Private keys** → **Generate a private key** → download the `.pem` file. The app expects the key in env as **base64**: encode the entire PEM contents (including `-----BEGIN/END-----` and newlines). Example (PowerShell): `[Convert]::ToBase64String([IO.File]::ReadAllBytes("path\to\your-app.pem"))`. Example (bash): `base64 -w0 path/to/your-app.pem`. Use the single-line base64 string as the value. |
| **github_app_installation_id** | ID of the **installation** of that app on the repo/org | After installing the app on the repo or org: **Organization** → **Third-party access** (or **GitHub Apps**) → **Configure** next to your app → the URL is `https://github.com/organizations/<org>/settings/installations/<INSTALLATION_ID>`. The number at the end is the Installation ID. For a **user** install: **GitHub → Settings → Applications → Installed GitHub Apps** → Configure → URL has the ID. Or via API (with a valid token): `GET https://api.github.com/repos/<owner>/<repo>/installation` — the JSON response has `"id": <INSTALLATION_ID>`. |
...
| **ADMIN_PASSWORD**            | infra                   | Password for admin analytics + CMS login                  | You choose it. Any strong secret. Exposed to the web app as the `ADMIN_PASSWORD` environment variable, and required by the Statistics tab in the admin panel and by `/api/auth` (Decap CMS login).                                                                                           |
| **AGENT_API_KEY**             | infra                   | Optional key for the AI agent API (`/api/agent/**`)       | You choose it, e.g. `openssl rand -hex 32`. Exposed to the web app as `AGENT_API_KEY`. If unset, the agent API is disabled.                                                                                                                              |
| **GH_APP_ID**                 | infra                   | GitHub App Application ID                                 | See **GitHub App credentials** above.                                                                                                                                                                                                                    |
| **GH_APP_PRIVATE_KEY_BASE64** | infra                   | GitHub App private key (base64-encoded PEM)               | See **GitHub App credentials** above. Must be the single-line base64 of the `.pem` file.                                                                                                                                                                 |
| **GH_APP_INSTALLATION_ID**    | infra                   | GitHub App installation ID on the repo/org                | See **GitHub App credentials** above.                                                                                                                                                                                                                    |
...
1. Add **ARM_CLIENT_ID**, **ARM_CLIENT_SECRET**, **ARM_SUBSCRIPTION_ID**, **ARM_TENANT_ID**, **POSTGRESQL_ADMIN_PASSWORD**, **ADMIN_PASSWORD**, **GH_APP_ID**, **GH_APP_PRIVATE_KEY_BASE64**, and **GH_APP_INSTALLATION_ID** so the **infra** workflow can run.
